...
export class TeamReportComponent implements OnInit {
  ...
  load(id: string): void {
    ...
    // Rewards can be assigned by teamId OR teamName (depending on UI usage),
    // so we fetch all rewards and filter locally for a robust report.
    forkJoin({
      ...
      next: ({ team, rewards }) => {
        ...
        const filtered = (rewards ?? []).filter(r => {
          const byId = (r.teamId ?? '') === id;
          const byName = (r.teamName ?? '').trim().toLowerCase() === teamKey;
          return byId || byName;
        });

        this.rewards = filtered.slice().sort((a, b) => {
